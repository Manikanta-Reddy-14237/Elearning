...
function TeacherDashboard() {
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            try {
                const response = await axios.get("https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/Teacherdata", {
                    headers: {
    ...
    const handleUpload = async (acceptedFiles) => {
        ...
        try {
            const uploadRoute = isVideoFile(acceptedFiles[0].name)
                ? "https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/teacher/upload/video"
                : "https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/teacher/upload";
    ...
    const handleDelete = async (fileId) => {
        ...
        if (confirmDelete) {
            try {
                await axios.post(
                    `http://elearning-khaki.vercel.app/teacher/delete/${fileId}`,
                    null,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
