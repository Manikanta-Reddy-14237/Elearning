...
function TeacherDashboard() {
    ...
    useEffect(() => {
        ...
        const fetchProfile = async () => {
            try {
                const response = await axios.get("http://elearning-khaki.vercel.app/Teacherdata", {
                    headers: {
    ...
    const handleUpload = async (acceptedFiles) => {
        ...
        try {
            const uploadRoute = isVideoFile(acceptedFiles[0].name)
                ? "http://elearning-khaki.vercel.app/teacher/upload/video"
                : "http://elearning-khaki.vercel.app/teacher/upload";
