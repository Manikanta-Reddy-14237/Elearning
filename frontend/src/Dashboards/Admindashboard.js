...
function AdminDashboard() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const token = localStorage.getItem('authToken');
                const studentData = await axios.get('https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/Studentsdata', {
                    headers: {
                        Authorization: `Bearer ${token}`, // Set the Authorization header
                    },
                });

                const teacherData = await axios.get('https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/Teachersdata', {
                    headers: {
                        Authorization: `Bearer ${token}`, // Set the Authorization header
                    },
                });
    ...
    const handleDelete = async (fileId, filename) => {
        ...
        if (confirmDelete) {
            try {
                await axios.post(
                    `https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/teacher/delete/${fileId}`,
                    null,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
