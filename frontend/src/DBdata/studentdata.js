...
function StudentDetails() {
    ...
    useEffect(() => {
        const fetchResources = async () => {
            try {
                ...
                var token = localStorage.getItem('authToken');
                const resourceData =await axios.get(`https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/Studentdata`, {
                    headers: {
    ...
    const removeStudent = async (studentId) => {
        try {
            await axios.post('https://elearningbackend-jg1p54tf9-manikantas-projects-f78ee616.vercel.app/stddelete', { studentId }, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
